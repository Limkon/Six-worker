...
async function parseSocks5Header(buffer) {
  if (buffer.byteLength < 6) return { hasError: true, message: "SOCKS5 request too short" };
  const view = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength);
  if (view.getUint8(0) !== 5) return { hasError: true, message: "Invalid SOCKS5 version" };
  if (view.getUint8(2) !== 0) return { hasError: true, message: "Invalid SOCKS5 RSV" };
...
function tryHandleSocks5Handshake(buffer, currentState, webSocket, ctx, log) {
  ...
  if (currentState === 1) {
    ...
    offset += pLen;
    const isValid = (user === ctx.userID || user === ctx.dynamicUUID || user === ctx.userIDLow) && (pass === ctx.userID || pass === ctx.dynamicUUID || pass === ctx.userIDLow);
    if (isValid) {
    ...
    async write(chunk, controller) {
      ...
      if (isConnected) {
        ...
        if (activeWriter) {
          ...
          if (remoteSocketWrapper.buffer.length < 1e3) {
